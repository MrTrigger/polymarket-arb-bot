...
impl PaperMode {
    ...
    pub async fn run(&mut self) -> Result<()> {
        ...
        let session = self.session_paths.take().unwrap_or_else(|| {
            common::create_session_dir("paper").expect("Failed to create session directory")
        });
        ...
        if self.config.observability.capture_decisions {
            ...
            match crate::strategy::decision_log::DecisionLogger::with_config(
                decisions_path.to_str().unwrap_or("decisions.csv"), "paper", Some(&log_config),
            ) {
                Ok(logger) => {
        ...
        // Always enable trades logging
        {
            ...
            match crate::strategy::trades_log::TradesLogger::new(&trades_path, "paper", self.config.initial_balance) {
                Ok(logger) => {
